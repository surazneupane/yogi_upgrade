...
class Slug
{
	...
	public function create_slug($string)
	{
		...
		helper('text','url','text');
		$string = mb_strtolower(url_title(convert_accented_characters($string), $this->_get_replacement()));
	...
	private function _check_uri($uri, $id = FALSE, $count = 0)
	{
		...
		// Setup the query
		$query = $db->table($this->table)->select($this->field)->where($this->field, $new_uri);

		if ($id) {
			$query = $query->where($this->id . ' !=', $id);
		}
