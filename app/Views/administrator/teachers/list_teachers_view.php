...
<script type="text/javascript">
	...
	function deleteTeacher(dataUrl) {
	...
	jQuery(document).ready(function () {
		...
		/*jQuery('#teacherlist-datTable').DataTable({
		  "ordering": false
		});*/
		
		// initialize the datatable 
		manageTable = jQuery('#teacherlist-datTable').DataTable({
			"bStateSave": true,
			"processing": true,
        	"serverSide": true,
			'ajax': baseURL + 'administrator/teachers/fetchTeacherData',
			'order': [],
			"columnDefs": [
		...
		jQuery("#checkAll").click(function () {
			jQuery('.teacher-list-form input:checkbox').not(this).prop('checked', this.checked);
		});
		
		jQuery('.btn-toolbar .btn-submit').click( function() {
			
			var dataCount = <?php echo count($teachers); ?>;
			if (jQuery('#teacherlist-datTable input[type="checkbox"]:checked').length != 0 && dataCount != 0) {
				...
				.then((willConfirm) => {
					if (willConfirm) {
						...
						jQuery('#teacher-list-form').submit();
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="box box-primary">
				...
				<?php echo form_open('',array('id'=>'teacher-list-form', 'class'=>'teacher-list-form')); ?>
			
		            <div class="box-header with-border">
		            	<div class="btn-toolbar">
							<a class="btn btn-success <?php if(!$ionAuthLibrary->in_access('teacher_access', 'add')) { ?>disabled<?php } ?>" href="<?php echo site_url('administrator/teachers/create');?>"><i class="fa fa-plus-circle"></i> Add Teacher</a>
							<?php 
							$publish_class = 'btn btn-submit btn-default';
							$disabled = '';
							if(!$ionAuthLibrary->in_access('teacher_access', 'update')) {  $publish_class .= ' disabled'; $disabled = 'disabled="disabled"'; } 
							?>
							...
							$disabled = '';
							if(!$ionAuthLibrary->in_access('teacher_access', 'update')) {  $unpublish_class .= ' disabled'; $disabled = 'disabled="disabled"'; } 
							?>
							...
							$disabled = '';
							if(!$ionAuthLibrary->in_access('teacher_access', 'delete')) {  $delete_class .= ' disabled'; $disabled = 'disabled="disabled"'; } 
							?>
...
//									$options[1] = 'Publish';
//									$options[0] = 'Unpublish';
//									echo form_dropdown('filter[status]', $options, $teachers_filter['status'], 'class="form-control select2" data-placeholder="- Select Status -"');
								?>
							</div>
							<div class="col-md-2">
		            ...
		            <div class="box-body">
							...
							<table id="teacherlist-datTable" class="table table-bordered table-striped teacherlist-datTable">
								<thead>
									<tr>
