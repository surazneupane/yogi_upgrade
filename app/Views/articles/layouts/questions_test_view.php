...
<div class="questions_test_view-layout-wrapper">
	...
	<div class="article-questions-div">
		...
		<?php if($question_result != '') { ?>
			...
			<!--<div class="answer-div alert alert-info">-->
			<div class="answer-div">
				<?php if($question_title == '') { ?>
				<h4>النتيجة : </h4>
				<?php } else { ?>
				<h4><?php echo $question_title; ?> : </h4>
				<?php } ?>
				<p><?php echo $question_result; ?></p>
			</div>
			<a href="<?php echo generateArticleLink($article['category_ids'], $article['alias']); ?>" class="btn btn-success">إعادة</a>
		...
		<?php } else { ?>
			...
			<div class="questions-form-div">
				<!-- form start -->
				<?php echo form_open('',array('id'=>'article-questions-form', 'class'=>'article-questions-form')); ?>
			
				<div class="question-div">
					<?php $i = 1; foreach ($questions_test_data AS $question_test_data) { ?>
					<div class="form-group">
						<label class="question-title"><?php echo $question_test_data->question; ?></label>
						<div class="radio">
		    				<label>
								<?php echo form_radio('question_'.$i, 'A', FALSE, ' class="Icheck"'); ?> <?php echo $question_test_data->answer_1; ?>
							</label>
						</div>
						<div class="radio">
		    				<label>
		    					<?php echo form_radio('question_'.$i, 'B', FALSE, ' class="Icheck"'); ?> <?php echo $question_test_data->answer_2; ?>
		    				</label>
		    			</div>
						<div class="radio">
		    				<label>
		    					<?php echo form_radio('question_'.$i, 'C', FALSE, ' class="Icheck"'); ?> <?php echo $question_test_data->answer_3; ?>
		    				</label>
		    			</div>
					</div>
					<?php $i++; } ?>
				...
				<?php echo form_submit('submit_questiontest', 'إرسال', 'id="submit_questiontest" class="btn btn-submit btn-success"'); ?>
				<?php echo form_reset('reset_questiontest', 'إعادة', 'id="reset_questiontest" class="btn btn-reset btn-danger"'); ?>
